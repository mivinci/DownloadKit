...
static void slash_argc_renderer(xLineCompletionEnv cenv, ReplCtx *ctx,
                                const char *token);
static void slash_cmd_verbose(ReplCtx *ctx, const char *args);
static void slash_argc_verbose(xLineCompletionEnv cenv, ReplCtx *ctx,
                               const char *token);
 ...
 *   /verbose on    → enable full output (default)
