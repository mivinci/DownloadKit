...
| [HTTPS Server](https_server.md) | xKit single-threaded HTTPS server vs Go `net/http` + `crypto/tls` — **512 K req/s** (HTTPS/2), TLS-bound parity on HTTPS/1.1 |
| [WebSocket Server](ws_server.md) | xKit single-threaded WS echo server vs Go gorilla/websocket, nhooyr/websocket, gobwas/ws — **220 K msg/s**, +18–27% faster than best Go library |
