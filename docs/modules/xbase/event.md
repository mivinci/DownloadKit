...
| `xEventLoopCancelSubmit` | `xErrno xEventLoopCancelSubmit(xEventLoop loop, xEventWork work)` | **Thread-safe** |
...
3. **Hybrid I/O + CPU Workloads** — Use `xEventLoopSubmit()` to offload CPU-intensive parsing or compression to a thread pool, then process results on the event loop thread where I/O state is safely accessible. Use `xEventLoopCancelSubmit()` to cancel pending work when the associated resource is being released.
...
- **Cancel offloaded work when releasing resources.** If you submit work via `xEventLoopSubmit()` and the associated resource (passed as `arg`) is about to be freed, use `xEventLoopCancelSubmit()` to prevent use-after-free. If cancel succeeds (`xErrno_Ok`), the arg is safe to free immediately. If it fails (`xErrno_InvalidState`), the work is already running — let `done_fn` handle cleanup.
