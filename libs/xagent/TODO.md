...
> Session/Query 拆分 **Step 1 与 Step 2 核心已落地**：`xAgentQuery` 已是
> first-class public handle，与 `xAgentSession` history 解耦，§8.1/§8.2/§8.3
...
> Step 3（`xAgentQueryCreateStandalone`）按
...
- [ ] **Async teardown**。
  现状：`xAgentSessionDestroy` 在 run 进行中调用时，只 `ai_provider_cancel`
...
- [x] **`on_finalizing` 钩子**。
  位置：session 销毁前（`xAgentSessionDestroy` 内）。Agent 注入。
...
- [x] **L1 数据写入机制**。
  ...
  核心设计：`xAgentSessionConf` 新增 `on_l1_preserve` 回调 + `l1_preserve_owner`，
  在以下三个时机触发：
...
- [ ] **L1 → L2 提取逻辑实现**。
  ...
  2. **提取策略**（在 Agent 层从 L1 全量消息中提取 L2 观察）。两阶段：
     - **规则先过**：检测硬特征（专有名词、数字、时间、URL、明确偏好词
       "我喜欢/讨厌/用…"），匹配则直接构造 `xAgentObservation`，
       confidence=1.0。
  ...
  3. **`on_finalizing` 内的汇总逻辑**。用一次 LLM call 做会话级摘要，
     输出若干 `xAgentObservation` + mood delta + 未完结话题。
...
- [ ] **记忆注入到新 session**。
  `xAgentCreateSession` 创建 session 时，从 L2 检索与当前上下文
...
- [ ] **定时 / 事件驱动唤醒框架**。
  ...
  调 `xAgentSessionInput` 发起主动对话。场景：
  - 定时提醒（"该复查代码了"）
  - 任务完成通知（后台 tool 执行完毕）
  - 主动建议（"我发现一个优化点"）
...
- [ ] **唤醒 session 的生命周期管理**。
  - 创建：agent 内部调 `xAgentCreateSession`，origin 设为
    `SystemSynthesized`
  - 运行：组装 agent nudge 消息，调 `xAgentSessionInput`
  - 销毁：对话结束后 `xAgentSessionDestroy`
...
- [ ] **`provider_anthropic.{h,c}`** — Anthropic Messages API。
  - 独立头文件 `<xagent/provider_anthropic.h>` + 自己的 `xAgentAnthropicConf`
...
- [ ] **Per-tool cancel token**。
  ...
  是同步函数，要么已返回要么还在卡。
  设计：
  - `xAgentToolHandler` 增加一个 `xAgentCancelToken` 参数（或通过 user_data
    间接拿到）
...
- [ ] **E2E smoke test**：起一个 local OpenAI-compatible endpoint
  ...
  **动手时机**：Session/Query 拆分的 Step 2 做完时顺手补——拆出
  `xAgentQuery` 之后恰好是测"单次 query 端到端"的最小单元。
...
- [ ] **Headers install 规则**。
  等 moo 顶层 `install` target 定下来后，xai 的公共头按规则补上
  （`<xagent/session.h>` / `<xagent/provider.h>` / `<xagent/message.h>` 等）。
...
在这一个钩子上统一拦截——不管是 `xAgentSessionInput` 刚开的新轮还是
...
- [x] **α：硬截断骨架（方案 TruncateOldest）**。
  ...
  同时暴露 hook 给 β 复用。
  拆成下列 commit：
  1. `xAgentBudgetPolicy` 枚举 + `xAgentSessionConf.budget`
...
放 **`xAgentSessionConf`**，不是 agent 也不是 query：
...
XDEF_ENUM(xAgentBudgetPolicy) {
  xAgentBudgetPolicy_Disabled       = 0,  /* 默认：不做任何检查 */
  xAgentBudgetPolicy_Error          = 1,  /* 超预算直接 on_error，最安全 */
  xAgentBudgetPolicy_TruncateOldest = 2,  /* α 方案：从最旧开始丢 */
  xAgentBudgetPolicy_Callback       = 3,  /* 回调 on_compact 让调用方决定 */
  xAgentBudgetPolicy_SummarizeOldest= 4,  /* β 方案：起 summary query */
};
...
实现位置：`session.c` 内 `static size_t estimate_tokens(const xAgentMessage *, size_t n)`，不暴露到头文件。
...
XDEF_STRUCT(xAgentSessionCallbacks) {
  ...
  xErrno (*on_compact)(const xAgentMessage *in, size_t n_in,
                       xAgentMessage **out, size_t *n_out,
...
- Usage 透传契约：`xAgentUsage{prompt,completion,total}` + `-1` 哨兵，
  跨轮累加在 session 层（详见 MEMORY.md）
- `xErrno_Busy` 映射到 xstrerror "resource busy"
- Context budget α + β：`xAgentBudgetPolicy` 枚举（Disabled / Error / TruncateOldest / SummarizeOldest / Auto） / `xAgentBudgetConf` 配置 /
...
- L1 采集机制落地：
  ...
  `xAgentCreateSession` 自动注入（agent 配置 `agent_id` + `data_dir` 时）
