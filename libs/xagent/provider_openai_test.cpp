...
static void on_tool(const xAgentContent *call, void *arg) {
  auto *r = static_cast<Recorder *>(arg);
...
TEST_F(OpenAIProviderTest, CreateRejectsNullArgs) {
  ...
  EXPECT_EQ(xAgentProviderOpenAICreate(loop, http, nullptr), nullptr);

  xAgentOpenAIConf no_key = {};
  EXPECT_EQ(xAgentProviderOpenAICreate(loop, http, &no_key), nullptr);
}
...
TEST_F(OpenAIProviderTest, StreamsTextAndFinishesWithEndTurn) {
  ...
  xAgentProvider pvd = make_provider(srv.base_url());
  ASSERT_NE(pvd, nullptr);
...
TEST_F(OpenAIProviderTest, CancelFiresDoneWithCancelledReason) {
  ...
  EXPECT_TRUE(rec.done_reason == xAgentProviderStop_Cancelled ||
              rec.done_reason == xAgentProviderStop_EndTurn);
...
TEST_F(OpenAIProviderTest, RequestBodyCarriesModelAndMessages) {
  ...
  xAgentContent sys_c  = xAgentContentText("be brief");
  xAgentMessage sys_m  = xAgentMessageFromContent(xAgentRole_System, &sys_c, 1);
 ...
 * encoding tool_calls request). SubmitConf.tools is `const xAgentTool **`,
 * i.e. an array of handle pointers. Before the fix the provider did
 *
 *     cJSON *t = oai_tool_to_json((xAgentTool)conf->tools[i]);
 *
...
TEST_F(OpenAIProviderTest, RequestBodyEncodesTools) {
  ...
  const xAgentTool *tools[] = {&t_time, &t_add};
  ...
  xAgentProviderDestroy(pvd);
  xAgentToolDestroy(t_add);
  xAgentToolDestroy(t_time);
  srv.join();
}
 ...
 * `xAgentProviderStop_Error`) so the session layer can distinguish
