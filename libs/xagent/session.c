...
static void sess_fwd_on_done(xAgentQuery q, xAgentDoneReason reason,
  ...
  if (s->compacting) {
    ...
    if (compact_ok) {
      /* Persist the about-to-be-replaced entries
      ...
      if (xArrayInsert(&s->history_arr, compact_start, &summary_entry)
        ...
        /* Persist the new summary entry so it is written to the
         * external store. Persist NOW because the original entries
         * it replaces are already gone — if we skip this, the
...
xAgentSession xAgentSessionCreate(xAgent agent, const xAgentSessionConf *conf) {
  ...
  /* Memory store for persistence: borrowed from conf. NULL when
   * no store is configured (in-memory-only sessions). */
  s->memory          = conf->memory;
  s->session_id_copy = conf->session_id_copy;
...
void xAgentSessionDestroy(xAgentSession sess) {
  ...
  /* Persist the full remaining history as a final snapshot before
   * the session is torn down. This ensures sessions that never
   * triggered a budget event still deliver their complete
   * conversation to the memory store.
   *
