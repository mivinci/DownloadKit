...
TEST(ResultTest, ConstructOkAndErrSameTypeAreDistinct) {
  xpp::Result<int, int> ok_r(xpp::ok, 1);
  xpp::Result<int, int> err_r(xpp::err, 2);

  EXPECT_TRUE(ok_r.is_ok());
  EXPECT_FALSE(ok_r.is_err());
  EXPECT_EQ(ok_r.unwrap(), 1);

  EXPECT_FALSE(err_r.is_ok());
  EXPECT_TRUE(err_r.is_err());
  EXPECT_EQ(err_r.unwrap_err(), 2);
}
...
TEST_F(TrackerResultTest, ConstructErrWithTrackerMoves) {
  {
    ...
    EXPECT_EQ(Tracker::copies, 0);
    EXPECT_GE(Tracker::moves, 1);
    EXPECT_EQ(r.unwrap_err().value, 22);
  }
  EXPECT_EQ(Tracker::alive, 0);
}
...
TEST_F(TrackerResultTest, UnwrapErrRvalueOverloadMoves) {
  ...
  Tracker::copies = 0;
  Tracker::moves  = 0;
  Tracker t       = std::move(r).unwrap_err();
...
TEST_F(TrackerResultTest, UnwrapOrRvalueOverloadMovesFallback) {
  ...
  Tracker::moves  = 0;
  Tracker fallback(42);
  Tracker out = std::move(r).unwrap_or(std::move(fallback));
...
TEST(ResultTransposeTest, ErrBecomesSomeErr) {
  ...
  auto                                       out = std::move(r).transpose();
  ASSERT_TRUE(out.is_some());
  EXPECT_TRUE(out.unwrap().is_err());
  EXPECT_EQ(out.unwrap().unwrap_err(), "boom");
}
...
TEST_F(TrackerResultTest, MapRvalueOnErrMovesError) {
  ...
  EXPECT_TRUE(s.is_err());
  EXPECT_EQ(s.unwrap_err().value, 90);
  EXPECT_EQ(Tracker::copies, 0);
}
...
TEST(ResultTest, ExpectErrReturnsErrWhenErr) {
  ...
  EXPECT_EQ(r.expect_err("must be err"), "bad");
}

TEST(ResultTest, ExpectErrRvalueMoves) {
  xpp::Result<int, std::string> r(xpp::err, std::string("bad"));
  std::string                   s = std::move(r).expect_err("must be err");
  EXPECT_EQ(s, "bad");
}
...
TEST(ResultTest, MapErrTransformsErr) {
  xpp::Result<int, int> r(xpp::err, 7);
  auto                  s = r.map_err([](int x) { return x + 100; });
  EXPECT_TRUE(s.is_err());
  EXPECT_EQ(s.unwrap_err(), 107);
}

TEST(ResultTest, MapErrPassesThroughOk) {
  xpp::Result<int, int> r(xpp::ok, 1);
  bool                  called = false;
  auto                  s      = r.map_err([&](int x) {
    called = true;
    return x;
  });
  EXPECT_FALSE(called);
...
TEST(ResultTest, MapErrChangesErrType) {
  ...
  auto                  s = r.map_err([](int x) { return std::to_string(x); });
  static_assert(std::is_same<decltype(s), xpp::Result<int, std::string>>::value, "");
  EXPECT_EQ(s.unwrap_err(), "9");
}
...
TEST_F(TrackerResultTest, MapErrRvalueMovesErr) {
  {
    ...
    Tracker::copies = 0;
    auto s          = std::move(r).map_err([](Tracker &&t) { return Tracker(t.value + 1); });
    EXPECT_EQ(Tracker::copies, 0);
    EXPECT_EQ(s.unwrap_err().value, 6);
  }
  EXPECT_EQ(Tracker::alive, 0);
}
...
TEST(ResultTest, AndThenChainsOk) {
  ...
  auto s = r.and_then([](int x) { return xpp::Result<int, std::string>(xpp::ok, x + 1); });
  EXPECT_TRUE(s.is_ok());
...
TEST(ResultTest, AndThenPassesThroughErr) {
  ...
  bool                          called = false;
  auto                          s      = r.and_then([&](int x) {
    called = true;
    return xpp::Result<int, std::string>(xpp::ok, x);
  });
  EXPECT_FALSE(called);
  EXPECT_TRUE(s.is_err());
  EXPECT_EQ(s.unwrap_err(), "nope");
}

TEST(ResultTest, AndThenFnCanReturnErr) {
  xpp::Result<int, std::string> r(xpp::ok, 4);
  auto s = r.and_then([](int) { return xpp::Result<int, std::string>(xpp::err, "bad"); });
  EXPECT_TRUE(s.is_err());
  EXPECT_EQ(s.unwrap_err(), "bad");
}
...
TEST(ResultVoidTest, AndThenOnVoidOk) {
  xpp::Result<void, std::string> r(xpp::ok);
  auto s = r.and_then([] { return xpp::Result<int, std::string>(xpp::ok, 9); });
  EXPECT_TRUE(s.is_ok());
...
TEST(ResultTest, OrElsePassesThroughOk) {
  xpp::Result<int, std::string> r(xpp::ok, 7);
  bool                          called = false;
  auto                          s      = r.or_else([&](const std::string &) {
    called = true;
...
TEST(ResultTest, OrElseRecoversFromErr) {
  xpp::Result<int, std::string> r(xpp::err, std::string("bad"));
  auto                          s = r.or_else([](const std::string &e) {
...
TEST(ResultTest, OrElseFnCanReturnErr) {
  xpp::Result<int, std::string> r(xpp::err, std::string("bad"));
  auto s = r.or_else([](const std::string &) { return xpp::Result<int, int>(xpp::err, 42); });
  EXPECT_TRUE(s.is_err());
  EXPECT_EQ(s.unwrap_err(), 42);
}

TEST(ResultVoidTest, OrElseVoidRecovers) {
  xpp::Result<void, int> r(xpp::err, 5);
  auto                   s = r.or_else([](int x) {
...
TEST(ResultTest, UnwrapOrElseReturnsValueWhenOk) {
  xpp::Result<int, std::string> r(xpp::ok, 5);
  EXPECT_EQ(std::move(r).unwrap_or_else([](std::string &&) { return 99; }), 5);
}

TEST(ResultTest, UnwrapOrElseUsesErrWhenErr) {
  xpp::Result<int, std::string> r(xpp::err, std::string("xyz"));
  int v = std::move(r).unwrap_or_else([](std::string &&e) { return static_cast<int>(e.size()); });
  EXPECT_EQ(v, 3);
}
...
TEST(ResultTest, InspectErrCallsFnWhenErr) {
  xpp::Result<int, std::string> r(xpp::err, std::string("bad"));
  std::string                   seen;
  r.inspect_err([&](const std::string &e) { seen = e; });
  EXPECT_EQ(seen, "bad");
  EXPECT_TRUE(r.is_err());
}
...
TEST(ResultTest, InspectErrSkipsWhenOk) {
  xpp::Result<int, std::string> r(xpp::ok, 1);
  bool                          called = false;
  r.inspect_err([&](const std::string &) { called = true; });
  EXPECT_FALSE(called);
}
...
TEST(ResultTest, InspectErrIsChainable) {
  ...
  std::string                   seen;
  auto                          s =
    std::move(r).inspect_err([&](const std::string &e) { seen = e; }).map_err([](std::string &&e) {
      return e + "!";
    });
  EXPECT_EQ(seen, "bad");
  EXPECT_EQ(s.unwrap_err(), "bad!");
...
TEST(ResultVoidTest, InspectErrOnVoid) {
  ...
  int                    seen = 0;
  r.inspect_err([&](const int &e) { seen = e; });
  EXPECT_EQ(seen, 9);
}
