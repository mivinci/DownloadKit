...
#define XAGENT_OAI_MAX_TOOL_CALLS 32
...
/*
 * Map xAgentRole → OpenAI role string.
 */
...
/*
 * Convert one xAgentMessage into a JSON object that matches OpenAI's
 ...
 *                                     content: "..."}.
 */
static cJSON *oai_message_to_json(const xAgentMessage *m) {
  cJSON *obj = cJSON_CreateObject();
  if (!obj) return NULL;
  cJSON_AddStringToObject(obj, "role", oai_role_str(m->role));

  if (m->role == xAgentRole_Tool) {
...
static cJSON *oai_tool_to_json(xAgentTool tool) {
  if (!tool) return NULL;
...
static char *oai_build_body(struct xOaiImpl_ *impl, const xAgentProviderSubmitConf *conf) {
  ...
  if (conf->tools && conf->tools_count > 0) {
    ...
    for (size_t i = 0; i < conf->tools_count; i++) {
      /* SubmitConf.tools is `const xAgentTool **` — an array of handle
...
static void oai_tool_calls_reset(struct xOaiImpl_ *impl) {
  for (int i = 0; i < XAGENT_OAI_MAX_TOOL_CALLS; i++) {
...
static void oai_on_sse_done(int curl_code, void *arg) {
  ...
  if (impl->cancelled) {
    impl->stop_reason = xAgentProviderStop_Cancelled;
    impl->stop_err    = xErrno_Ok;
...
static const xAgentProviderVtable kOpenAIVtable = {
  .submit  = oai_submit,
  .cancel  = oai_cancel,
  .destroy = oai_destroy,
};
