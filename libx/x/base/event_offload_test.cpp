...
TEST_F(EventOffloadTest, BasicOffload) {
  ...
  ASSERT_EQ(xEventLoopSubmit(loop, group, basic_work, basic_done, &ctx, nullptr), xErrno_Ok);
...
TEST_F(EventOffloadTest, FireAndForget) {
  ...
  ASSERT_EQ(xEventLoopSubmit(loop, group, work_fn, nullptr, &work_done, nullptr), xErrno_Ok);
...
TEST_F(EventOffloadTest, NullLoopReturnsError) {
  EXPECT_EQ(xEventLoopSubmit(nullptr, group, basic_work, basic_done, nullptr, nullptr),
            xErrno_InvalidArg);
}
...
TEST_F(EventOffloadTest, NullGroupUsesGlobal) {
  ...
  ASSERT_EQ(xEventLoopSubmit(loop, nullptr, basic_work, basic_done, &ctx, nullptr), xErrno_Ok);
...
TEST_F(EventOffloadTest, ResultPassedToDoneFn) {
  ...
  ASSERT_EQ(xEventLoopSubmit(loop, group, work_fn, done_fn, &out, nullptr), xErrno_Ok);
...
TEST_F(EventOffloadTest, SubmitFailsWhenGroupFull) {
  ...
  /* Fill the queue */
  xEventLoopSubmit(
    loop, small, [](void *) -> void * { return nullptr; }, nullptr, nullptr, nullptr);
...
TEST_F(EventOffloadTest, CancelSubmitOutHandle) {
  ...
  xEventWork work = nullptr;
  ASSERT_EQ(xEventLoopSubmit(
              loop, group, [](void *) -> void * { return nullptr; }, nullptr, nullptr, &work),
            xErrno_Ok);
