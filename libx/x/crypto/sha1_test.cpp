...
TEST(Sha1, MillionAs) {
  ...
  uint8_t     digest[XCRYPTO_SHA1_DIGEST_SIZE];
  ASSERT_EQ(xSha1((const uint8_t *)input.data(), input.size(), digest),
            xErrno_Ok);
  EXPECT_EQ(hex(digest, XCRYPTO_SHA1_DIGEST_SIZE),
...
TEST(Sha1, StreamingMatchesOneShot) {
  ...
  ASSERT_EQ(xSha1Update(&ctx, (const uint8_t *)parts[0], strlen(parts[0])),
            xErrno_Ok);
  ASSERT_EQ(xSha1Update(&ctx, (const uint8_t *)parts[1], strlen(parts[1])),
            xErrno_Ok);
