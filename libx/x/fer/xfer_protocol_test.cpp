...
TEST(XferProtocol, FileMetaBufferTooSmall) {
  ...
  size_t  len = 0;
  EXPECT_EQ(xTransferEncodeFileMeta(&meta, buf, sizeof(buf), &len), xErrno_InvalidArg);
}

TEST(XferProtocol, FileMetaNullArgs) {
  uint8_t buf[512];
  size_t  len = 0;
  EXPECT_EQ(xTransferEncodeFileMeta(NULL, buf, sizeof(buf), &len), xErrno_InvalidArg);
...
TEST(XferProtocol, EncodeDecodeChunkHeader) {
  ...
  ASSERT_EQ(xTransferDecodeChunkHeader(msg + 1, 6, &chunk_id, &payload, &payload_len), xErrno_Ok);
...
TEST(XferProtocol, FileDoneBufferTooSmall) {
  ...
  size_t  len = 0;
  EXPECT_EQ(xTransferEncodeFileDone(&done, buf, sizeof(buf), &len), xErrno_InvalidArg);
}
...
TEST(XferProtocol, EncodeDecodeFileResume) {
  ...
  uint8_t buf[64];
  size_t  len = 0;
  ASSERT_EQ(xTransferEncodeFileResume(&resume, buf, sizeof(buf), &len), xErrno_Ok);
...
TEST(XferProtocol, FileResumeBufferTooSmall) {
  ...
  size_t  len = 0;
  EXPECT_EQ(xTransferEncodeFileResume(&resume, buf, sizeof(buf), &len), xErrno_InvalidArg);
}

TEST(XferProtocol, FileResumeNullArgs) {
  uint8_t buf[64];
  size_t  len = 0;
  EXPECT_EQ(xTransferEncodeFileResume(NULL, buf, sizeof(buf), &len), xErrno_InvalidArg);
...
TEST(XferProtocol, FileResumeLargeBitmap) {
  ...
  size_t               len = 0;
  ASSERT_EQ(xTransferEncodeFileResume(&resume, buf.data(), buf.size(), &len), xErrno_Ok);
  ASSERT_EQ(len, 9u + bitmap_len);

  xTransferFileResume decoded;
  ASSERT_EQ(xTransferDecodeFileResume(buf.data() + 1, len - 1, &decoded), xErrno_Ok);
