...
static void sender_send_next_chunk(xTransfer_ *impl) {
  ...
  while (impl->send_next_chunk < impl->send_total_chunks && impl->send_has_resume &&
  ...
  if (impl->send_next_chunk >= impl->send_total_chunks) {
    ...
    if (xfer_compute_file_sha1(impl->vfs, impl->send_filepath, impl->send_sha1) != xErrno_Ok) {
      xfer_report_error(impl, xErrno_SysError, "Failed to compute file SHA-1");
      return;
    }
  ...
  if (buffered > XFER_SEND_HIGH_WATER_MARK) {
    ...
    xDataChannelSetBufferedAmountLowThreshold(impl->dc, XFER_SEND_LOW_WATER_MARK);
    XDEBUG("[xfer] sender paused: buffered=%zu high_water=%d", buffered, XFER_SEND_HIGH_WATER_MARK);
    return;
  }
  ...
  if (!buf) {
    xfer_report_error(impl, xErrno_NoMemory, "Failed to allocate chunk buffer");
    return;
  }
  ...
  /* Read file data */
  size_t nread    = 0;
  xErrno read_err = impl->vfs->pread(impl->vfs->ctx, impl->send_handle, buf + hdr_len,
                                     impl->send_chunk_size, offset, &nread);
  ...
  if (err == xErrno_Again) {
    ...
    impl->send_paused = true;
    xDataChannelSetBufferedAmountLowThreshold(impl->dc, XFER_SEND_LOW_WATER_MARK);
    XDEBUG("[xfer] sender paused: SCTP EAGAIN on chunk %u", impl->send_next_chunk);
    return;
  }
  if (err != xErrno_Ok) {
  ...
  impl->send_next_chunk++;
  uint64_t transferred = (uint64_t)(impl->send_next_chunk - 1) * impl->send_chunk_size + nread;
...
void sender_on_dc_message(xDataChannel channel, xDataChannelMsgType type, const uint8_t *data,
  ...
  case XFER_MSG_ACK: {
    ...
    if (ack.status == 0) {
      ...
      xfer_report_error(impl, xErrno_Unknown, "Receiver reported verification failure");
    }
    break;
  }
  ...
  case XFER_MSG_FILE_RESUME: {
    ...
    if (xTransferDecodeFileResume(payload, payload_len, &resume) != xErrno_Ok) {
    ...
    if (resume.total_chunks != impl->send_total_chunks) {
      xfer_report_error(impl, xErrno_InvalidArg, "FILE_RESUME total_chunks mismatch");
      return;
    }

    /* Load bitmap from the received data */
    if (resume.bitmap && resume.bitmap_len > 0) {
      xErrno err = xBitmapInit(&impl->send_resume_bitmap, resume.total_chunks);
      if (err != xErrno_Ok) {
      ...
      if (copy_len > impl->send_resume_bitmap.nbytes) copy_len = impl->send_resume_bitmap.nbytes;
      ...
      (void)already_done;
      XDEBUG("[xfer] sender: resume bitmap received, %u/%u chunks done", already_done,
             resume.total_chunks);
    }
