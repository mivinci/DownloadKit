...
xErrno xWsConnect(xEventLoop loop, const xWsConnectConf *conf, const xWsCallbacks *callbacks,
                  void *arg) {
  ...
  c->timer      = xEventLoopTimerAfter(loop, connector_timeout_cb, c, (uint64_t)c->timeout_ms);
  ...
  c->dns_query = xDnsResolve(loop, hostname, port_str, &hints, connector_dns_cb, c);
  if (!c->dns_query) {
    connector_destroy(c);
