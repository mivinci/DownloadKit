...
ic_private attr_t attr_update_with(attr_t oldattr, attr_t newattr) {
  ...
  if (newattr.x.italic != IC_NONE) {
    attr.x.italic = newattr.x.italic;
  }
  if (newattr.x.reverse != IC_NONE) {
    attr.x.reverse = newattr.x.reverse;
  }
  if (newattr.x.underline != IC_NONE) {
    attr.x.underline = newattr.x.underline;
  }
  return attr;
}
...
ic_private attr_t attr_from_sgr(const char *s, ssize_t len) {
  ...
  for (ssize_t i = 0; i < len && s[i] != 0; i++) {
    ...
    case 3:
      ...
      break;
    case 4:
      attr.x.underline = IC_ON;
      break;
    case 7:
      attr.x.reverse = IC_ON;
      break;
    case 22:
      attr.x.bold = IC_OFF;
      break;
...
static bool attrbuf_ensure_capacity(attrbuf_t *ab, ssize_t needed) {
  ...
  ssize_t newcap =
    (ab->capacity <= 0 ? 240 : (ab->capacity > 1000 ? ab->capacity + 1000 : 2 * ab->capacity));
  if (needed > newcap) {
    newcap = needed;
  }
  attr_t *newattrs = (attr_t *)realloc(ab->attrs, to_size_t(newcap) * sizeof(attr_t));
