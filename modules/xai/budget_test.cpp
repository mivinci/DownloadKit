...
TEST(XaiBudgetEstimate, ThinkingCountsSameAsText) {
  ...
  EXPECT_EQ(ai_budget_estimate_tokens(&a, 1),
            ai_budget_estimate_tokens(&b, 1));
}
...
TEST(XaiBudgetEstimate, MixedSliceSumsPerEntry) {
  ...
  EXPECT_EQ(ai_budget_estimate_tokens(seq, n),
            42u / XAI_BUDGET_BYTES_PER_TOKEN
...
TEST(XaiBudgetFindNth, NoUserEntries) {
  ...
  EXPECT_EQ(ai_budget_find_nth_user_turn(seq, 2, 0),
            XAI_BUDGET_NO_SUCH_TURN);
}
...
TEST(XaiBudgetFindNth, OutOfRangeReturnsSentinel) {
  ...
  EXPECT_EQ(ai_budget_find_nth_user_turn(seq, 1, 1),
            XAI_BUDGET_NO_SUCH_TURN);
  EXPECT_EQ(ai_budget_find_nth_user_turn(seq, 1, 42),
            XAI_BUDGET_NO_SUCH_TURN);
}
...
TEST(XaiBudgetEarliestKeep, BoundaryIsAlwaysAUserRoleIndex) {
  ...
  xAiSessionMsg_ seq[] = {
      MakeText(xAiRole_User, "u1"),
      MakeToolUse("a", "t", "{}"),
      MakeToolResult("a", "r", 0),
      MakeText(xAiRole_User, "u2"),
      MakeText(xAiRole_Assistant, "a2"),
      MakeText(xAiRole_User, "u3"),
  };
  const size_t n = sizeof(seq) / sizeof(seq[0]);
  ...
  for (const auto &c : cases) {
    ...
    if (idx != 0) {
      EXPECT_EQ(seq[idx].role, xAiRole_User)
          << "boundary at idx " << idx
          << " must be a User-role entry (keep_recent="
          << c.keep_recent << ")";
    }
  }
}
