...
TEST(XaiMessage, ContentTextBasic) {
  ...
  EXPECT_EQ(c.type, xAiContentType_Text);
...
TEST(XaiMessage, ContentTextNullIsAccepted) {
  ...
  xAiContent c = xAiContentText(nullptr);
  EXPECT_EQ(c.type, xAiContentType_Text);
...
TEST(XaiMessage, ContentTextUtf8LengthIsBytes) {
  ...
  xAiContent  c = xAiContentText(s);
  EXPECT_EQ(c.u.text.len, 6u);
}

/* ── xAiMessageFromContent ────────────────────────────────────────────── */

TEST(XaiMessage, MessageFromContentCarriesFields) {
  xAiContent blocks[2];
  blocks[0] = xAiContentText("a");
  blocks[1] = xAiContentText("bb");

  xAiMessage m = xAiMessageFromContent(xAiRole_Assistant, blocks, 2);
  EXPECT_EQ(m.role, xAiRole_Assistant);
  EXPECT_EQ(m.contents, blocks); /* borrows, does not copy */
  EXPECT_EQ(m.n, 2u);
}

TEST(XaiMessage, MessageFromContentEmpty) {
  xAiMessage m = xAiMessageFromContent(xAiRole_System, nullptr, 0);
  EXPECT_EQ(m.role, xAiRole_System);
...
TEST(XaiMessage, FromTextProducesUserRole) {
  xAiMessage m = xAiMessageFromText("ping");
  EXPECT_EQ(m.role, xAiRole_User);
  ASSERT_EQ(m.n, 1u);
  ASSERT_NE(m.contents, nullptr);
  EXPECT_EQ(m.contents[0].type, xAiContentType_Text);
...
TEST(XaiMessage, FromTextSecondCallOverwritesTlsSlot) {
  ...
  xAiMessage second = xAiMessageFromText("second");
...
TEST(XaiMessage, FromTextIsThreadLocal) {
  ...
  const xAiContent *a_slot = nullptr;
  const xAiContent *b_slot = nullptr;
  std::atomic<int>  captured{0};
  std::atomic<bool> release{false};

  auto body = [&](const char *tag, const xAiContent **out) {
    xAiMessage m = xAiMessageFromText(tag);
    *out         = m.contents;
