 ...
 *
 * Declares the concrete layout of xAiProvider (vtable pointer + ctx
...
#ifndef XAI_PROVIDER_PRIVATE_H
#define XAI_PROVIDER_PRIVATE_H
 ...
 * this struct and fills in @p vt + @p ctx. The public xAiProvider
 * handle is just a pointer to this struct cast to void*.
 */
struct xAiProvider_ {
  const xAiProviderVtable *vt;  /**< Static vtable (never NULL).         */
