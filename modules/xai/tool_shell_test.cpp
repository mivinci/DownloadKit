 ...
 *
 * tool_shell_test.cpp - Unit tests for xAiToolShellCreate / xAiToolShellStdinCreate
 */
...
TEST(ShellTool, CreateWithDefaults) {
  ...
  xAiTool tool = xAiToolShellCreate(loop, nullptr);
...
TEST(ShellTool, CreateWithConf) {
  ...
  xAiTool tool = xAiToolShellCreate(loop, &conf);
  ASSERT_NE(tool, nullptr);
...
TEST(ShellTool, CreateNullLoop) {
  EXPECT_EQ(xAiToolShellCreate(nullptr, nullptr), nullptr);
}

TEST(ShellTool, DestroyNullIsNoop) {
  xAiToolDestroy(nullptr);
}
...
TEST(ShellTool, UserDataIsShellCtx) {
  ...
  EXPECT_NE(xAiToolUserData(tool), nullptr);
...
TEST(ShellStdinTool, CreateFromShellTool) {
  ...
  xAiTool shell_tool = xAiToolShellCreate(loop, nullptr);
  ASSERT_NE(shell_tool, nullptr);

  xAiTool stdin_tool = xAiToolShellStdinCreate(shell_tool);
  ...
  EXPECT_NE(xAiToolUserData(stdin_tool), nullptr);
  /* Both tools share the same ShellCtx */
  EXPECT_EQ(xAiToolUserData(stdin_tool), xAiToolUserData(shell_tool));

  xAiToolDestroy(stdin_tool);
  xAiToolDestroy(shell_tool);
  xEventLoopDestroy(loop);
}

TEST(ShellStdinTool, CreateNullShellTool) {
  EXPECT_EQ(xAiToolShellStdinCreate(nullptr), nullptr);
}
...
TEST(ShellStdinTool, DestroyOrderIndependent) {
  ...
  EXPECT_NE(xAiToolUserData(shell_tool), nullptr);
...
TEST(ShellStdinTool, HandlerInvalidArgs) {
  ...
  /* NULL args_json */
  xAiContent in1 = {};
  in1.type                 = xAiContentType_ToolUse;
  ...
  xAiContent in2 = {};
  in2.type                 = xAiContentType_ToolUse;
  ...
  /* Empty input string */
  xAiContent in3 = {};
  in3.type                 = xAiContentType_ToolUse;
  ...
  /* Malformed JSON */
  xAiContent in4 = {};
  in4.type                 = xAiContentType_ToolUse;
...
TEST(ShellIntegration, ShellAndStdinShareContext) {
  ...
  void *shell_ud = xAiToolUserData(shell_tool);
  void *stdin_ud = xAiToolUserData(stdin_tool);
...
TEST(ShellIntegration, MultipleStdinToolsShareContext) {
  ...
  xAiTool stdin1 = xAiToolShellStdinCreate(shell_tool);
  xAiTool stdin2 = xAiToolShellStdinCreate(shell_tool);
  ASSERT_NE(stdin1, nullptr);
  ASSERT_NE(stdin2, nullptr);

  EXPECT_EQ(xAiToolUserData(stdin1), xAiToolUserData(shell_tool));
  EXPECT_EQ(xAiToolUserData(stdin2), xAiToolUserData(shell_tool));

  xAiToolDestroy(stdin2);
  xAiToolDestroy(stdin1);
  xAiToolDestroy(shell_tool);
  xEventLoopDestroy(loop);
}
