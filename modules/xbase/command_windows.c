 ...
 *
 * PTY mode (xCommandInput_Pty) returns xErrno_NotSupported — a future
 * ConPTY-based implementation can be added under XK_HAS_CONPTY.
 */
...
static void on_pipe_chunk(void *arg) {
  ...
  if (exec->destroying) { free(chunk->data); free(chunk); return; }
...
static DWORD WINAPI pipe_reader_thread(LPVOID param) {
  ...
  for (;;) {
    ...
    if (!chunk->data) { free(chunk); break; }
...
static void on_process_exit_posted(void *arg) {
  ...
  if (exec->destroying) { free(info); return; }
...
static void cmd_fire_done(struct xCommandExecutor_ *exec) {
  ...
  /* Close pipe handles */
  if (exec->hStdoutRead)  { CloseHandle(exec->hStdoutRead);  exec->hStdoutRead  = NULL; }
  if (exec->hStderrRead)  { CloseHandle(exec->hStderrRead);  exec->hStderrRead  = NULL; }
  if (exec->hStdinWrite)  { CloseHandle(exec->hStdinWrite);  exec->hStdinWrite  = NULL; }
  if (exec->stdin_fd >= 0) { _close(exec->stdin_fd); exec->stdin_fd = -1; }

  /* Close process handles */
  if (exec->hProcess)       { CloseHandle(exec->hProcess);       exec->hProcess       = NULL; }
  if (exec->hPrimaryThread) { CloseHandle(exec->hPrimaryThread); exec->hPrimaryThread = NULL; }
...
static void on_timeout(void *arg) {
  ...
  exec->cancel_timer = xEventLoopTimerAfter(exec->loop, on_cancel_grace,
                                             exec, CMD_CANCEL_GRACE_MS);
}
...
xErrno xCommandExecutorSubmit(xCommandExecutor           exec_,
  ...
  /* PTY mode not supported on Windows yet */
  if (conf->input_mode == xCommandInput_Pty) return xErrno_NotSupported;
  ...
  if (conf->stdout_mode != xCommandOutput_Discard) {
    ...
    hNulStdout = CreateFileW(L"NUL", GENERIC_WRITE, 0, &sa,
  ...
  if (conf->stderr_mode != xCommandOutput_Discard) {
    ...
    hNulStderr = CreateFileW(L"NUL", GENERIC_WRITE, 0, &sa,
                             OPEN_EXISTING, 0, NULL);
  ...
  si.hStdOutput = (conf->stdout_mode != xCommandOutput_Discard)
                    ? hStdoutWrite : hNulStdout;
  si.hStdError  = (conf->stderr_mode != xCommandOutput_Discard)
                    ? hStderrWrite : hNulStderr;
  ...
  if (conf->cwd) {
    ...
    if (wcwd)
      MultiByteToWideChar(CP_UTF8, 0, conf->cwd, -1, wcwd, wlen);
  }
  ...
  BOOL ok = CreateProcessW(
    NULL,             /* lpApplicationName — search PATH via cmdline */
  ...
  if (exec->hStdoutRead) {
    ...
    if (!exec->hStdoutThread) { free(ctx); goto fail_process; }
  } else {
    exec->stdout_eof = 1;
  }
  ...
  if (exec->hStderrRead) {
    ...
    if (!exec->hStderrThread) { free(ctx); goto fail_process; }
  } else {
    exec->stderr_eof = 1;
  }
...
fail_process:
  ...
  TerminateProcess(exec->hProcess, 1);
  WaitForSingleObject(exec->hProcess, 5000);
  if (exec->hWait) { UnregisterWaitEx(exec->hWait, INVALID_HANDLE_VALUE); exec->hWait = NULL; }
  if (exec->hStdoutThread) { WaitForSingleObject(exec->hStdoutThread, 5000); CloseHandle(exec->hStdoutThread); exec->hStdoutThread = NULL; }
  if (exec->hStderrThread) { WaitForSingleObject(exec->hStderrThread, 5000); CloseHandle(exec->hStderrThread); exec->hStderrThread = NULL; }
  if (exec->hStdoutRead)   { CloseHandle(exec->hStdoutRead);   exec->hStdoutRead   = NULL; }
  if (exec->hStderrRead)   { CloseHandle(exec->hStderrRead);   exec->hStderrRead   = NULL; }
  if (exec->hStdinWrite)   { CloseHandle(exec->hStdinWrite);   exec->hStdinWrite   = NULL; }
  if (exec->stdin_fd >= 0)  { _close(exec->stdin_fd); exec->stdin_fd = -1; }
...
fail:
  ...
  if (exec->hStderrRead)  { CloseHandle(exec->hStderrRead);  exec->hStderrRead  = NULL; }
  if (exec->stdin_fd >= 0) { _close(exec->stdin_fd); exec->stdin_fd = -1; }
  if (exec->stdout_buf) { xStringDestroy(exec->stdout_buf); exec->stdout_buf = NULL; }
  if (exec->stderr_buf) { xStringDestroy(exec->stderr_buf); exec->stderr_buf = NULL; }
  exec->state = xCommandExecutorState_Idle;
...
xErrno xCommandExecutorCancel(xCommandExecutor exec_) {
  ...
  if (exec->state != xCommandExecutorState_Running)
...
int xCommandExecutorPid(xCommandExecutor exec_) {
  if (!exec_) return -1;
  struct xCommandExecutor_ *exec = (struct xCommandExecutor_ *)exec_;
  return (exec->state != xCommandExecutorState_Idle)
           ? (int)exec->child_pid : -1;
}
...
int xCommandExecutorPtyFd(xCommandExecutor exec_) {
  (void)exec_;
  return -1; /* PTY not supported on Windows */
}
