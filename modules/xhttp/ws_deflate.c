...
xWsDeflateCtx *xWsDeflateCreate(const xWsDeflateParams *params, int is_client) {
  ...
  if (deflateInit2(&ctx->deflate_strm, Z_DEFAULT_COMPRESSION, Z_DEFLATED,
                   -send_wbits, 8, Z_DEFAULT_STRATEGY) != Z_OK) {
    free(ctx);
    return NULL;
  }
  ctx->deflate_init = 1;
...
int xWsDeflateCompress(xWsDeflateCtx *ctx, const uint8_t *in, size_t in_len,
  ...
  size_t buf_sz =
    (in_len > 0) ? (in_len + in_len / 10 + 64) : DEFLATE_INIT_BUF_SIZE;
...
static int param_match(const char *p, size_t len, const char *name) {
  return len == strlen(name) && xnet_strncasecmp(p, name, len) == 0;
}
...
int xWsDeflateParseOffer(const char *value, size_t value_len,
  ...
  while (p < end) {
    ...
    if (name_len == 18 && xnet_strncasecmp(p, "permessage-deflate", 18) == 0) {
      found           = 1;
      params->enabled = 1;
      ...
      while (pp < ext_end) {
        ...
        const char *next_semi =
          (const char *)memchr(pp, ';', (size_t)(ext_end - pp));
        const char *param_end = next_semi ? next_semi : ext_end;

        /* Find '=' if present */
        const char *eq =
          (const char *)memchr(pp, '=', (size_t)(param_end - pp));
...
int xWsDeflateBuildServerResponse(const xWsDeflateParams *params, char *buf,
  ...
  if (params->server_no_context_takeover) {
    int r =
      snprintf(buf + n, buf_sz - (size_t)n, "; server_no_context_takeover");
    if (r < 0) return -1;
    n += r;
  }
  if (params->client_no_context_takeover) {
    int r =
      snprintf(buf + n, buf_sz - (size_t)n, "; client_no_context_takeover");
    if (r < 0) return -1;
    n += r;
  }
