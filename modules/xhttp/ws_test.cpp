...
static std::vector<uint8_t> build_client_frame(uint8_t fin, uint8_t opcode,
...
TEST(WsFrame, ParseTextFrame) {
  ...
  auto    frame_data =
    build_client_frame(1, XWS_OPCODE_TEXT, "Hello", 5, mask_key);
...
TEST(WsFrame, ParseBinaryFrame) {
  ...
  auto    frame_data =
    build_client_frame(1, XWS_OPCODE_BINARY, data, sizeof(data), mask_key);
...
TEST(WsFrame, ParseEmptyFrame) {
  ...
  auto    frame_data =
    build_client_frame(1, XWS_OPCODE_TEXT, nullptr, 0, mask_key);
...
TEST(WsFrame, ParseMediumPayload) {
  ...
  auto frame_data = build_client_frame(1, XWS_OPCODE_TEXT, payload.data(),
...
TEST(WsFrame, ParseCloseFrame) {
  ...
  uint8_t mask_key[4]     = {0x00, 0x00, 0x00, 0x00};
  auto    frame_data =
    build_client_frame(1, XWS_OPCODE_CLOSE, close_payload, 2, mask_key);
...
static void ws_test_on_close(xWsConn conn, uint16_t code, const char *reason,
                             size_t len, void *arg) {
  (void)conn;
  (void)reason;
  (void)len;
...
static bool ws_send_frame(xnet_socket_t fd, uint8_t fin, uint8_t opcode,
  ...
  xnet_ssize_t n = send(fd, (const char *)frame.data(), (int)frame.size(), 0);
  return n == (xnet_ssize_t)frame.size();
}
...
static RecvFrame ws_recv_frame(xnet_socket_t fd, int timeout_ms = 2000) {
  ...
  uint8_t      hdr[2];
  xnet_ssize_t n = recv(fd, (char *)hdr, 2, MSG_WAITALL);
  ...
  if (len7 == 126) {
    uint8_t ext[2];
    if (recv(fd, (char *)ext, 2, MSG_WAITALL) != 2) return result;
    payload_len = ((uint64_t)ext[0] << 8) | ext[1];
  } else if (len7 == 127) {
    uint8_t ext[8];
    if (recv(fd, (char *)ext, 8, MSG_WAITALL) != 8) return result;
    payload_len = 0;
    for (int i = 0; i < 8; i++)
  ...
  if (masked) {
    uint8_t mask[4];
    if (recv(fd, (char *)mask, 4, MSG_WAITALL) != 4) return result;
  }
  ...
  if (payload_len > 0) {
    ...
    if (n != (xnet_ssize_t)payload_len) return result;
  }
...
protected:
  ...
  void SetUpWsRoute(const std::string &path = "/ws") {
    ...
    xErrno      err =
      xHttpServerRoute(server, pattern.c_str(), ws_upgrade_handler, &ws_ctx);
