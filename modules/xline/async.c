...
typedef enum xline_async_state_e {
  ...
  XLINE_ASYNC_DONE_LINE,  // user pressed Enter (or cancel returning empty string)
...
ic_private bool xline_async_is_live(void) { return g_live_session != NULL; }
...
ic_public xLineStepResult xLineStep(xLineHandle* h) {
  ...
  switch (h->state) {
    ...
    case XLINE_ASYNC_RUNNING:
      break;
    case XLINE_ASYNC_DONE_LINE:  return XLINE_STEP_LINE;
    case XLINE_ASYNC_DONE_EOF:   return XLINE_STEP_EOF;
    case XLINE_ASYNC_DONE_ERROR: return XLINE_STEP_ERROR;
  }
...
ic_public void xLinePrintAbove(xLineHandle* h, const char* s) {
  ...
  term_write(env->term, "\r\x1b[J");  // CSI 0J — clear from cursor to end of screen
