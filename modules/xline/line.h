 ...
 *
 * line.h - Interactive line editing public API
 */
#pragma once
#ifndef XLINE_LINE_H
#define XLINE_LINE_H
...
void xLineCompleteWord(xLineCompletionEnv *cenv, const char *prefix,
                       xLineCompleterFn *fun, xLineIsCharClassFn *is_word_char);
...
void xLineCompleteQwordEx(xLineCompletionEnv *cenv, const char *prefix,
                          xLineCompleterFn    fun,
                          xLineIsCharClassFn *is_word_char, char escape_char,
 ...
 * user input.
 */
typedef void(xLineHighlightFn)(xLineHighlightEnv *henv, const char *input,
                               void *arg);
 ...
 * Can be called in a `xLineHighlightFn` callback to colorize the `input` using
 ...
 */
char *xLineReadlineEx(const char *prompt_text, xLineCompleterFn *completer,
                      void *completer_arg, xLineHighlightFn *highlighter,
 ...
 */
long xLineMatchToken(const char *s, long pos, xLineIsCharClassFn *is_token_char,
                     const char *token);
...
long xLineMatchAnyToken(const char *s, long pos,
                        xLineIsCharClassFn *is_token_char, const char **tokens);
...
/* ── FD-level async line editing. These primitives let a caller drive line
 * editing from its own event loop (poll/select/kqueue). While a session is
 * pending the caller may print chunks above the current edit line using
 * xLinePrintAbove() without corrupting the user's input. Call sequence:
 * xLineHandle* h = xLineBegin("> "); int fd = xLineFd(h); register `fd` for
 * read-readiness in your event loop... for (;;) { poll/select/... on fd (and on
 * your other sources); xLineStepResult r = xLineStep(h); if (r ==
 * XLINE_STEP_LINE) { char* s = xLineTake(h); ...; xLineFree(s); break; } if (r
 * == XLINE_STEP_EOF || r == XLINE_STEP_ERROR) break; r == XLINE_STEP_PENDING:
 * keep looping } xLineEnd(h); The synchronous xLineReadline() is internally
 * implemented in terms of these primitives; the two modes MUST NOT be used at
 * the same time — calling xLineReadline() while a handle is live returns NULL
 * and logs a warning ── */
