...
struct term_s {
  ...
  COORD
  hcon_save_cursor; // saved cursor position (for escape sequence emulation)
#endif
};
...
ic_private void term_set_attr(term_t *term, attr_t attr) {
  ...
  if (attr.x.color != term->attr.x.color && attr.x.color != IC_COLOR_NONE) {
    ...
    if (term->palette < ANSIRGB && color_is_rgb(attr.x.color)) {
      term->attr.x.color =
        attr.x.color; // actual color may have been approximated but we keep the
                      // actual color to avoid updating every time
    }
  }
...
ic_private term_t *term_new(alloc_t *mem, tty_t *tty, bool nocolor, bool silent,
  ...
  if (!term->nocolor) {
    ...
    // otherwise check for some specific terminals
    ...
    else if (getenv("VSCODE_PID") != NULL) {
      term->palette = ANSIRGB;
    } // vscode terminal
    else {
      ...
      if (ic_contains(eterm, "truecolor") || ic_contains(eterm, "direct") ||
        ...
        term->palette = ANSI8;
                 ...
                 ic_contains(eterm, "nocolor") || ic_contains(eterm, "dumb")) {
        term->palette = MONOCHROME;
      }
    }
...
static void term_append_utf8(term_t *term, const char *s, ssize_t len) {
  ...
  if (unicode_is_raw(uchr, &c)) {
    // write bytes as is; this also ensure that on non-utf8 terminals characters
    // between 0x80-0xFF go through _as is_ due to the qutf8 encoding.
...
static void term_append_buf(term_t *term, const char *s, ssize_t len) {
  ...
  while (pos < len) {
    ...
    // handle escape sequence (note: str_next_ofs considers whole CSI escape
    // sequences at a time)
...
static void term_erase_line(term_t *term, ssize_t mode) {
  ...
  FillConsoleOutputAttribute(term->hcon, term->hcon_default_attr, (DWORD)length,
                             start, &written);
...
static void term_set_win_attr(term_t *term, attr_t ta) {
  ...
  if (ta.x.bgcolor != IC_COLOR_NONE) {
    if (ta.x.bgcolor >= IC_ANSI_BLACK && ta.x.bgcolor <= IC_ANSI_SILVER) {
      attr =
        (attr & 0xFF0F) | (WORD)(attr_color[ta.x.bgcolor - IC_ANSI_BLACK] << 4);
...
static bool term_write_direct(term_t *term, const char *s, ssize_t len) {
  ...
  if ((term->hcon_mode & ENABLE_VIRTUAL_TERMINAL_PROCESSING) != 0) {
    // use the builtin virtual terminal processing. (enables truecolor for
    // example)
    ...
    while (pos < len) {
      ...
      // (We don't need to handle utf-8 separately as we set the codepage to
      // always be in utf-8 mode)
      ssize_t nonctrl = 0;
      ssize_t next;
...
static bool term_esc_query_color_raw(term_t *term, int color_idx,
                                     uint32_t *color) {
  char buf[128 + 1];
...
ic_private void term_start_raw(term_t *term) {
  ...
  if (term->hcon_mode == 0) {
    ...
    // use escape sequence handling if available and the terminal supports it
    // (so we can use rgb colors in Windows terminal) Unfortunately, in plain
    // powershell, we can successfully enable terminal processing but it still
    // fails to render correctly; so we require the palette be large enough
    // (like in Windows Terminal)
