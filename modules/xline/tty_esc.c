...
This is a bit tricky there are many variants to encode keys as escape sequences,
see for example:
...
In ECMA48 `special? (number (';' modifiers)?)?` is the more liberal
`[\x30-\x3F]*` but that seems never used for key codes. If the number (vtcode or
unicode) or the modifiers are not given, we assume these are '1'. We then accept
...
Moreover, we translate the following special cases that do not fit into the
above grammar. First we translate away special starter sequences:
---------------------------------------------------------------------
  ESC '[' '[' ..      ~>  ESC '[' ..                  # Linux sometimes uses
extra '[' for CSI ESC '[' 'O' ..      ~>  ESC 'O' ..                  # Linux
sometimes uses extra '[' for SS3 ESC 'o' ..          ~>  ESC 'O' .. # Eterm:
ctrl + SS3 ESC '?' ..          ~>  ESC 'O' ..                  # vt52 treated as
SS3
...
---------------------------------------------------------------------
  ...
  ESC .. [^@$]        ~>  ESC .. '~'                  # ETerm,xrvt,urxt: ^ =
ctrl, $ = shift, @ = alt ESC '[' [a-d]       ~>  ESC '[' '1' ';' '2' [A-D]   #
Eterm shift+<cursor> ESC 'O' [1-9] final ~>  ESC 'O' '1' ';' [1-9] final #
modifiers as parameter 1 (like on Haiku) ESC '[' [1-9] [^~u] ~>  ESC 'O' '1' ';'
[1-9] final # modifiers as parameter 1
...
static code_t esc_decode_xterm(uint8_t xcode) {
  ...
  case 'O':
    return KEY_F3;
  case 'P':
    return KEY_F4; // note: differs from
                   // <https://en.wikipedia.org/wiki/ANSI_escape_code#CSI_(Control_Sequence_Introducer)_sequences>
  case 'Q':
    return KEY_F5;
  case 'R':
...
ic_private code_t tty_read_esc(tty_t *tty, long esc_initial_timeout,
  ...
  // treat ESC ESC as Alt modifier (macOS sends ESC ESC [ [A-D] for
  // alt-<cursor>)
  if (peek == KEY_ESC) {
