...
public:
  ...
  void perform_io() {
    ...
    while (poll(&pfd, 1, 10) > 0 && (pfd.revents & POLLIN)) {
      ...
      ssize_t rv = nghttp2_session_mem_recv(session_, (const uint8_t *)buf, (size_t)n);
      if (rv < 0) break;

      /* Send any frames generated by processing received data (e.g. SETTINGS ACK) */
      flush_session();
    }
  }
...
private:
  ...
  static ssize_t post_read_cb(nghttp2_session *session, int32_t stream_id,
...
static void h2_hello_handler(xHttpResponseWriter w, const xHttpRequest *req,
                              void *arg) {
  (void)req;
  (void)arg;
