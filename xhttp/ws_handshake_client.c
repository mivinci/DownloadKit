...
int xWsClientBuildUpgradeRequest(xIOBuffer *io,
  ...
  int key_len = xWsBase64Encode(nonce, sizeof(nonce),
  ...
  xWsSHA1((const unsigned char *)concat,
          kl + sizeof(WS_GUID) - 1, sha1);

  if (xWsBase64Encode(sha1, XWS_SHA1_DIGEST_SIZE,
                      accept_out, accept_sz) < 0)
    return -1;
  ...
  if (url->port && url->port_len > 0) {
    host_len = snprintf(host_buf, sizeof(host_buf),
                        "%.*s:%.*s",
  ...
  if (host_len < 0 || (size_t)host_len >= sizeof(host_buf))
    return -1;
  ...
  char ext_hdr[192];
  snprintf(ext_hdr, sizeof(ext_hdr),
  ...
  if (xIOBufferAppend(io, req, (size_t)req_len) != xErrno_Ok)
    return -1;
...
int xWsClientValidateUpgradeResponse(const char *data,
  ...
  size_t vlen;
  const char *upgrade = find_resp_header(hdr_start, hdr_len,
                                         "Upgrade", &vlen);
  if (!upgrade ||
      !header_token_match(upgrade, vlen, "websocket"))
    return -1;

  /* Check Connection: Upgrade */
  const char *conn = find_resp_header(hdr_start, hdr_len,
                                      "Connection", &vlen);
  if (!conn ||
      !header_token_match(conn, vlen, "Upgrade"))
    return -1;

  /* Check Sec-WebSocket-Accept */
  const char *accept = find_resp_header(hdr_start, hdr_len,
  ...
  if (vlen != expect_len ||
      memcmp(accept, expected_accept, expect_len) != 0)
    return -1;
...
int xWsClientParseDeflateResponse(const char *data,
  ...
  size_t vlen;
  const char *ext = find_resp_header(
    hdr_start, hdr_len,
    "Sec-WebSocket-Extensions", &vlen);
