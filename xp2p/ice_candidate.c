...
int xIceCandidateTypePref(xIceCandidateType type) {
  switch (type) {
    case xIceCandidateType_Host:  return XICE_TYPE_PREF_HOST;
    case xIceCandidateType_Srflx: return XICE_TYPE_PREF_SRFLX;
    case xIceCandidateType_Prflx: return XICE_TYPE_PREF_PRFLX;
    case xIceCandidateType_Relay: return XICE_TYPE_PREF_RELAY;
    default: return 0;
  }
}
  ...
  if (stun_server) {
    if (stun_server->sa_family == AF_INET) {
      ...
      const struct sockaddr_in6 *s6 =
          (const struct sockaddr_in6 *)stun_server;
...
const char *xIceCandidateTypeStr(xIceCandidateType type) {
  switch (type) {
    case xIceCandidateType_Host:  return "host";
    case xIceCandidateType_Srflx: return "srflx";
    case xIceCandidateType_Prflx: return "prflx";
    case xIceCandidateType_Relay: return "relay";
...
xErrno xIceCandidateTypeFromStr(const char *str, xIceCandidateType *type) {
  ...
  if (strcmp(str, "host") == 0) { *type = xIceCandidateType_Host; return xErrno_Ok; }
  if (strcmp(str, "srflx") == 0) { *type = xIceCandidateType_Srflx; return xErrno_Ok; }
  if (strcmp(str, "prflx") == 0) { *type = xIceCandidateType_Prflx; return xErrno_Ok; }
  if (strcmp(str, "relay") == 0) { *type = xIceCandidateType_Relay; return xErrno_Ok; }
