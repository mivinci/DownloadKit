...
static void mock_on_complete(const xStunMsg *msg,
                              const struct sockaddr *addr, void *arg) {
  (void)addr;
  (void)arg;
...
TEST_F(StunTxnTest, SendCreatesTransaction) {
  xErrno err = xStunTxnMgrSend(&mgr, xStunMsgType_BindingRequest,
                                 NULL, 0,
...
TEST_F(StunTxnTest, ResponseMatchesTransaction) {
  ...
  bool matched = xStunTxnMgrOnResponse(&mgr, &decoded_resp,
                                        resp_buf, resp_len,
                                        (struct sockaddr *)&dest);
  EXPECT_TRUE(matched);
...
TEST_F(StunTxnTest, CancelAllCleansUp) {
  ...
  ASSERT_EQ(xStunTxnMgrSend(&mgr, xStunMsgType_BindingRequest,
                              NULL, 0, (struct sockaddr *)&dest,
...
TEST_F(StunTxnTest, SendRawWorks) {
  ...
  xErrno err = xStunTxnMgrSendRaw(&mgr, msg_buf, msg_len,
                                    (struct sockaddr *)&dest,
                                    mock_send, NULL,
                                    mock_on_complete, NULL);
  ASSERT_EQ(err, xErrno_Ok);
...
TEST_F(StunTxnTest, MultipleTransactionsMultiplex) {
  ...
  xStunMsg response;
  xStunMsgInit(&response, xStunMsgType_BindingResponse,
               mgr.txns[1]->txn_id);
  uint8_t resp_buf[64];
  int resp_len = xStunMsgEncode(&response, resp_buf, sizeof(resp_buf));
  ASSERT_GT(resp_len, 0);
